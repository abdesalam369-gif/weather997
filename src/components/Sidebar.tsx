...
const MetricCard = ({ icon: Icon, label, value, unit, colorClass, isCollapsed }: any) => (
  <div className={cn(
    ...
    <div className={cn("flex items-center justify-between flex-row-reverse w-full", isCollapsed && "justify-center")}>
      ...
      {!isCollapsed && <div className="text-[8px] font-bold text-text-muted uppercase tracking-[0.1em]">{label}</div>}
    </div>
...
export const Sidebar: React.FC<SidebarProps> = ({ weather, alerts, isCollapsed }) => {
  return (
    <div className={cn(
      "h-full bg-brand-panel border-r border-brand-border flex flex-col p-6 overflow-y-auto text-right transition-all duration-300 ease-in-out",
      isCollapsed ? "w-0 p-0 opacity-0 pointer-events-none" : "w-72"
    )}>
      {!isCollapsed && (
        <>
          <div className="mb-8 relative">
            <div className="absolute -top-4 -right-4 w-20 h-20 bg-brand-accent/5 blur-3xl rounded-full" />
            <h2 className="text-[9px] font-bold text-brand-accent uppercase tracking-[0.2em] mb-1.5">بلدية مؤتة والمزار</h2>
            <h1 className="text-2xl font-bold text-text-primary tracking-tight leading-tight">غرفة الطوارئ<br />المركزية</h1>
            <div className="flex items-center justify-end gap-2 mt-3">
              <span className="text-[9px] font-bold text-text-muted uppercase tracking-widest text-right">تحديث مباشر: {new Date().toLocaleTimeString('ar-JO')}</span>
          ...
          <div className="flex-1">
            <h3 className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-4">التنبيهات النشطة</h3>
            <div className="space-y-3">
              {alerts.map((alert, i) => (
                <div 
                  key={i} 
                  className={cn(
                    "p-3 rounded-xl border flex gap-3 items-start flex-row-reverse relative overflow-hidden group transition-all hover:bg-brand-panel/80",
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    {alert.severity === 'red' ? <ShieldAlert className="w-4 h-4" /> : 
                     alert.severity === 'yellow' ? <AlertTriangle className="w-4 h-4" /> : 
                     <CheckCircle2 className="w-4 h-4" />}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className={cn(
                      "text-[9px] font-bold uppercase tracking-[0.1em] mb-0.5",
                    ...
                    <p className="text-xs text-text-secondary leading-relaxed font-medium line-clamp-3">{alert.message}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-8 pt-6 border-t border-brand-border">
            <div className="flex justify-between items-center mb-4 flex-row-reverse">
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">مصدر البيانات</span>
              <span className="text-[10px] font-bold text-text-primary uppercase">OpenWeatherMap API</span>
            </div>
